...
<div width="100%" align="center"><a href="https://github.com/programmer-salam/laravel-tips" align="left"><img align="left" width="45%" src="https://github-readme-stats.vercel.app/api/pin/?username=programmer-salam&repo=laravel-tips&title_color=0891b2&text_color=ffffff&icon_color=0891b2&bg_color=1c1917&hide_border=true&locale=en" /></a><a href="https://github.com/programmer-salam/Laravel-Push-Notification-To-Web-Apps" align="right"><img align="right" width="45%" src="https://github-readme-stats.vercel.app/api/pin/?username=programmer-salam&repo=Laravel-Push-Notification-To-Web-Apps&title_color=0891b2&text_color=ffffff&icon_color=0891b2&bg_color=1c1917&hide_border=true&locale=en" /></a></div>
    ...
    public function finishRegistration(Request $request)
    {
        ...
        $selectedInterests = Session::get('selected_interests');
        $selectedGender = Session::get('selected_gender');
        ...
        if (!$data) {
            Log::error('Session data is missing');
            return redirect()->back()->withErrors(['error' => 'Session data not found. Please try again.']);
        }
    
        // Retrieve profile information from the request
        $profileData = $request->only(['username', 'number', 'bio', 'image']);
        ...
        $imageName = 'Myavatar-' . $user->id . '.' . $image->getClientOriginalExtension();
        $image->move(public_path('UserImages'), $imageName);
    
        // Start query logging
        DB::listen(function ($query) {
            Log::info('Query executed: ', ['query' => $query->sql, 'bindings' => $query->bindings]);
        });
        ...
        try {
            ...
            $userProfile = UserProfile::updateOrCreate(
                ['user_id' => $user->id], // Find profile by user_id or create a new one
                [
                    'user_id' => $user->id,
                    'career_interest' => is_array($selectedInterests) ? implode(',', $selectedInterests) : $selectedInterests ?? null,
                    'gender' => $selectedGender ?? null,
                    'username' => $profileData['username'],
                    'phone_number' => $profileData['number'],
                    'bio' => $profileData['bio'],
                    'user_image' => 'UserImages/' . $imageName,
                    'first_question' => $data['security1'],
                    'first_answer' => $data['answer1'],
                    'second_question' => $data['security2'],
                    'second_answer' => $data['answer2'],
                ]
            );
    
            // Force the attributes to be considered as changed
            $userProfile->first_question = $data['security1'];
            $userProfile->first_answer = $data['answer1'];
            $userProfile->second_question = $data['security2'];
            $userProfile->second_answer = $data['answer2'];
            $userProfile->save();
    
            DB::commit();
    
            // Clear session data
            Session::forget(['selected_interests', 'selected_gender', 'userData']);
        ...
        // Redirect to the successful page or any other page
